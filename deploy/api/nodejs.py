...
def install_nodejs(request):
    if request.method == 'POST':
        ...
        nodejs_version = request.POST.get('nodejs_version', 'ERROR')
        ...
        if ip == port == username == password == nodejs_version == install_path == close_firewall == close_selinux == "ERROR":
            data = {
                'msg': '从前端获取参数失败',
                'status': 500
            }
