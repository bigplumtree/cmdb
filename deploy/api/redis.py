...
def install_redis(request):
    if request.method == 'POST':
        ...
        redis_workpath = request.POST.get('redis_workpath', 'ERROR')
        redis_port = request.POST.get('redis_port', 'ERROR')
        redis_password = request.POST.get('redis_password', 'ERROR')
        close_selinux = request.POST.get('close_selinux', 'ERROR')
        close_firewall = request.POST.get('close_firewall', 'ERROR')

        if ip == port == username == password == redis_workpath == redis_port == redis_password == close_firewall == close_selinux == "ERROR":
            data = {
                'msg': '从前端获取参数失败',
                'status': 500
            }
            ...
            try:
                ...
                ans.run_playbook(playbook_path='deploy/playbook/redis.yml', extra_vars=None)
