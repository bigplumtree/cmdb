...
def install_minio(request):
    if request.method == 'POST':
        ...
        minio_workpath = request.POST.get('minio_workpath', 'ERROR')
        minio_port = request.POST.get('minio_port', 'ERROR')
        minio_username = request.POST.get('minio_username', 'ERROR')
        minio_password = request.POST.get('minio_password', 'ERROR')


        if ip == port == username == password == minio_password == close_firewall == close_selinux == minio_username == minio_port == minio_workpath == "ERROR":
            data = {
                'msg': '从前端获取参数失败',
                'status': 500
            }
            ...
            try:
                ...
                ans.run_playbook(playbook_path='deploy/playbook/minio.yml', extra_vars=None)
